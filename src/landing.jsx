    ...
    <div className="relative w-full text-white overflow-x-hidden">
      ...
      <nav
        className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 backdrop-blur-md ${
          solidNav
            ? "bg-white/80 py-4" // white with transparency when scrolled
            : "bg-black/0 py-6" // transparent black at top
        }`}
      >
            ...
            {["home", "about", "skills", "projects", "contact"].map((sec) => (
              <li key={sec} className="relative">
                ...
                >
                  ...
                  {/* Gradient underline like Skills */}
                  {activeSection === sec && (
                    <span
                      className={`absolute left-0 bottom-0 h-[2px] w-full rounded-full transition-all duration-300 ${
                        solidNav
                          ? "bg-gradient-to-r from-purple-500 via-yellow-400 to-transparent"
                          : "bg-gradient-to-r from-purple-500 via-yellow-400 to-transparent"
                      }`}
                    />
                  )}
